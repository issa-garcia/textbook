...
objects, such as trays, then when we stack them, we will get the 3D array
...
The object in the foreground of the image (with 10, 11, 12 on its topmost row) is the first *plane* of the 3D array, and contains the values from the `first_2d` array. The object in the background of the image (with 100, 101, 102 on its topmost row) is the second plane, and contains the values from the `second_2d` array.
...
Currently the 3D array only contains zeros, so next we make the `first_2d`
array be the first 4 by 3 plane of the 3D array:
...
```{python}
# Set all rows, all columns for plane 0 to be the first_2d array
into_3d[:, :, 0] = first_2d
```

The image below shows how the `into_3d` array now looks, with the `first_2d` array as the first 3 by 4 plane:


![](images/two_d_slices_first_plane.png)


Next we make the `second_2d` array be the second 4 by 3 plane of the 3D array:
...
```{python}
# Set all rows, all columns for plane 1 to be the second_2d array
into_3d[:, :, 1] = second_2d
```
We have just stacked the `first_2d` and `second_2d` arrays into the `into_3d` array. The image below shows how the index commands we just used in order to do this relate to the planes of the `into_3d` array, and to the values that are now in each plane.

`into_3d[:, :, 0]` can be read as *'all rows, all columns of plane 0'*.

`into_3d[:, :, 1]` can be read as *'all rows, all columns of plane 1'*.
...
![](images/two_d_slices_edited.png)
