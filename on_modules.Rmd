...
```{python}
#For extra points
def vol_means_extrapoints(image_fname):
    img = nib.load(image_fname)
    data = img.get_fdata()
    vector_data = np.reshape(data, (np.prod(data.shape[:3]), data.shape[-1]))
    means_vectorized = np.mean(vector_data, axis=0)
    return np.array(means_vectorized)
```
...
```{python}
#For extra points
my_means_2 = vol_means_extrapoints(data_fname)
plt.plot(my_means_2)
```

```{python}
assert np.allclose(my_means, my_means_2)
```
