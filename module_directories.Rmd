...
We use the [os.path](path_manipulation.Rmd) routines to check if the directory
exists, and the `rmtree` function from the `shutil` module to remove it, if it
does:
...
if op.isdir('mydirmod'):
    shutil.rmtree('mydirmod')
```
